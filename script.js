...
function buildConcernSummary(input, pathway) {
  ...
  if (pathway === fallback) {
    return `You searched: “${cleanInput}.” To make this easier to explain, try focusing on when it started, how often it happens, how severe it feels, and whether it is affecting daily life.`;
  }
  return `You searched: “${cleanInput}.” This is worth taking seriously if it is recurring, worsening, affecting your comfort, or changing how you live day to day. The next step is to notice the pattern, prepare the right words, and decide what support may be appropriate.`;
}

function getThinkingPrompts(pathway) {
  const universal = [
    "When did this first start?",
    "Is it new, recurring, or getting worse?",
    "Is it affecting work, school, sleep, sex, exercise or daily life?",
    "Does it follow a pattern, such as your cycle, stress, food, movement or medication?",
    "Have you noticed any other symptoms happening at the same time?"
  ];

  const prompts = {
    "Menstrual health": [
      "How many days does the bleeding or pain last?",
      "Are you changing pads, tampons, cups or period underwear more often than usual?",
      "Are there clots, dizziness, nausea, headaches or fatigue?",
      "Do symptoms stop you from going to work, school or social plans?"
    ],
    "Pelvic pain": [
      "Is the pain sharp, dull, cramping, burning or deep?",
      "Does it happen during sex, after sex, during your period, or when using the toilet?",
      "Is the pain one-sided or central?",
      "Do you have bowel, bladder, bleeding or discharge changes?"
    ],
    "Hormones and energy": [
      "Is the fatigue constant or does it come in waves?",
      "Have your periods, weight, skin, hair, mood or sleep changed?",
      "Do symptoms feel worse at a certain point in your cycle?",
      "Have you had iron, ferritin, thyroid, B12 or vitamin D checked before?"
    ],
    "Perimenopause and menopause": [
      "Have your periods changed in timing, flow or predictability?",
      "Are sleep, mood, hot flushes, night sweats or brain fog affecting daily life?",
      "Are there vaginal, urinary or sexual symptoms you have not mentioned yet?",
      "What outcome would you want from an appointment: reassurance, tests, HRT discussion, or symptom support?"
    ],
    "Autoimmune and chronic conditions": [
      "Do symptoms come in flares?",
      "Do you notice joint pain, rashes, digestive symptoms, hair loss, weakness or brain fog?",
      "Is there a family history of autoimmune, thyroid, coeliac or inflammatory conditions?",
      "Do symptoms change around your period, stress, infections, food or rest?"
    ],
    "Sexual and vaginal health": [
      "Is there itching, burning, unusual discharge, smell or pain?",
      "Is sex painful at the entrance, deeper in the pelvis, or afterwards?",
      "Have symptoms started after sex, antibiotics, contraception changes or a new product?",
      "Would an STI screen, swab or urine test give you peace of mind?"
    ],
    "Pregnancy and postpartum": [
      "How many weeks pregnant or postpartum are you?",
      "Is there bleeding, severe pain, fever, reduced movements or feeling faint?",
      "Is this affecting feeding, sleep, mood, recovery or your ability to cope?",
      "Do you know who to contact: midwife, maternity triage, GP or health visitor?"
    ],
    "Mental wellbeing": [
      "Are these feelings linked to your cycle or present most days?",
      "Are sleep, appetite, motivation, concentration or relationships affected?",
      "Do you feel safe right now?",
      "Who could you tell today so you are not carrying this alone?"
    ]
  };

  return [...(prompts[pathway.name] || []), ...universal].slice(0, 8);
}

function renderThinkingPrompts(pathway) {
  const prompts = getThinkingPrompts(pathway);
  document.getElementById("thinkingList").innerHTML = prompts.map((prompt, index) => `
    <label class="thinking-item">
      <input type="checkbox" data-thinking-index="${index}">
      <span>${prompt}</span>
    </label>
  `).join("");
}
...
function buildAppointmentSummary(input, pathway) {
  const cleanInput = input.replace(/[?.!]+$/g, "");
  return `I have been experiencing: “${cleanInput}.” I would like to understand what could be contributing to this, whether any checks, tests or referrals would be appropriate, and what options could help. It would be helpful to talk through how often this is happening, whether there are any patterns, and what symptoms I should monitor or seek urgent help for.`;
}
